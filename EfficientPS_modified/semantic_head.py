...
class SemanticHead(nn.Module):
    ...
    def forward(self, inputs, targets={}):
        ...
        if targets is not None:
            losses = self.loss(outputs, targets.squeeze(1))
        
        return self.softmax(outputs), losses['semantic_loss']
    ...
    def loss(self, inputs, targets):
        ...
        We keep 25% of each image apply the weight and then compute the mean.
        Args:
            inputs: logits [B, nb_class, H, W]
            targets: class indices [B, H, W], dtype long
        """
        ...
        size = loss.shape[1]
        max_id = int(math.ceil(size * 0.25))
        ...
        return {'semantic_loss': kept_loss}
